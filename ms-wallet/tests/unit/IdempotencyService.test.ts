...
describe('IdempotencyService', () => {
  it('returns stored response for repeated requests', async () => {
    ...
    const result = await service.execute({
      key: 'key-1',
      scope: 'transactions:create:user-1',
      requestHash: 'hash-1',
      handler,
    });
    ...
    expect(replay.responseBody).toEqual({ id: 'tx-1' });
  ...
  it('throws conflict when payload changes', async () => {
    ...
    await service.execute({
      ...
      scope: 'transactions:create:user-1',
      requestHash: 'hash-1',
      handler: async () => ({ statusCode: 201, responseBody: { id: 'tx-1' } }),
    });
    ...
    await expect(
      service.execute({
        ...
        scope: 'transactions:create:user-1',
        requestHash: 'hash-2',
        handler: async () => ({ statusCode: 201, responseBody: { id: 'tx-2' } }),
      })
  ...
  it('deletes record if handler throws', async () => {
    ...
    const existing = await repository.findByKey('transactions:create:user-1', 'key-1');
